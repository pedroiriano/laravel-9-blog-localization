    ...
    <body>
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
            <div class="container-fluid">
                <a class="navbar-brand" href="#">Pedro Iriano</a>
                        ...
                        <li class="nav-item dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                                @foreach (LaravelLocalization::getSupportedLanguagesKeys() as $locale)
                                    <li>
                                        <a class="dropdown-item" href="{{ LaravelLocalization::getLocalizedURL($locale) }}">
                                            {{ strtoupper($locale) }}
                                        </a>
                                    </li>
                                @endforeach
                ...
                <div class="col-3">
                    <div class="card text-white bg-dark mb-3" style="max-width: 18rem;">
                        <div class="card-header">
                            {{ $data->postTranslation->first()->locale }}
                        </div>
                        <div class="card-body">
                            <h5 class="card-title">
                                {{ $data->postTranslation->first()->title }}
                            </h5>
                            <p class="card-text">
                                {{ Str::limit( strip_tags( $data->postTranslation->first()->content ), 100 ) }}
                            </p>
                        </div>
                    </div>
                </div>
